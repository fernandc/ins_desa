...
@section("title")
    Resumen de solicitud de información
@endsection
    ...
    <div class="table-responsive">
        ...
        <table class="table table-sm" style="text-align: center;" id="flag">
            <thead class="thead-light">
                <tr>
                    <th scope="col">Numero de matrícula</th>
                    ...
                    <th scope="col">Edad de años al 31 de Marzo</th>
                    <th scope="col">Curso</th>
                    <th scope="col">Domicilio</th>
                    <th scope="col">Comuna</th>
                    <th scope="col">Procedencia (Establecimiento)</th>
                    <th scope="col">Fecha de incorporación</th>
                    <th scope="col">Fecha de retiro</th>
                    <th scope="col">Causal de retiro</th>
                    <th scope="col">Nivel educacional padre</th>
                    <th scope="col">Nivel educacional madre</th>
                    <th scope="col">Nombre apoderado</th>
                    <th scope="col">Dirección apoderado</th>
                    <th scope="col">Avisar en caso de emergencia</th>
                </tr>
            </thead>
            <tbody>
            </tbody>
            ...
            <table class="table table-sm" style="text-align: center;" id="list_students_{{$i}}">
                ...
                <tbody>
                    @foreach($listas[$i] as $row)
                        @php
                        $fecha_nacimiento = $row["fecha_nacimiento_stu"];
                        $fecha_nacimiento_dt = new DateTime($fecha_nacimiento);
                        $fecha_actual_dt = new DateTime(Session::get('period')."-03-31");
                        $diferencia = $fecha_nacimiento_dt->diff($fecha_actual_dt);
                        $edad = $diferencia->y;
                        $direccion = explode(" - " , $row["direccion"]);
                        @endphp
                        <tr>
                            <td>{{$row["numero_matricula"]}} </td>
                            <td>{{$row["apellido_paterno_stu"]}} </td>
                            <td>{{$row["apellido_materno_stu"]}} </td>
                            <td>{{$row["nombres_stu"]}} </td>
                            <td>{{$row["sexo_stu"] == 'V' ? 'M' : 'F'}} </td>
                            <td>{{$row["dni_stu"]}} </td>
                            <td>
                                @php echo date("d-m-Y", strtotime($row["fecha_nacimiento_stu"])); @endphp
                            </td>
                            <td>{{$edad}} </td>
                            <td>{{$row["curso_abreviado"]}} </td>
                            <td>{{$direccion[0]}} </td>
                            <td>{{$direccion[1]}} </td>
                            <td>{{$row["establecimiento_previo"]}} </td>
                            <td>
                                Fecha incorporación
                            </td>
                            <td>
                                {{$row["fecha_retiro"] != null ? date("d-m-Y", strtotime($row["fecha_retiro"])) : ""}}
                            </td>
                            <td>
                                Causal de retiro
                            </td>
